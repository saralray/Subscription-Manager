...
interface NotificationConfigState {
  ...
  setDiscordConfig: (config: { webhook_url: string; validated?: boolean }) => void;
  clearChannelConfig: (channel: 'telegram' | 'email' | 'discord') => void;
  clearAllConfigs: () => void;

  // 验证状态管理
  setChannelValidated: (channel: 'telegram' | 'email' | 'discord', validated: boolean) => void;
}
      ...
      setDiscordConfig: (config) => {
        set((state) => ({
          channelConfigs: {
            ...state.channelConfigs,
            discord: {
              ...state.channelConfigs.discord,
              webhook_url: config.webhook_url,
              validated: config.validated ?? state.channelConfigs.discord?.validated,
              lastValidated: config.validated ? new Date().toISOString() : state.channelConfigs.discord?.lastValidated,
            },
          },
        }));
      },
